...
public:
    ...
    std::optional<AccessorT> As() const {
    ...
    std::string GetID() const {
        return IsValid() ? m_feature->featureID : "";
    }
    ...
    bool IsType() const {
        return IsValid() && std::dynamic_pointer_cast<const FeatureT>(m_feature) != nullptr;
    }

    // --- 底层指针访问（供子类使用） ---
    std::shared_ptr<const CFeatureBase> GetRaw() const {
        return m_feature;
    }
};
