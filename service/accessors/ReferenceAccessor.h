...
public:
    explicit ReferenceAccessor(std::shared_ptr<const CRefEntityBase> ref = nullptr)
        : m_ref(std::move(ref)) {}
    ...
    RefType GetRefType() const {
        if (!IsValid()) return RefType::FEATURE_DATUM_PLANE;
    ...
    // --- 底层访问 ---
    std::shared_ptr<const CRefEntityBase> GetRaw() const {
        return m_ref;
    }
    ...
    std::shared_ptr<const RefT> GetAs() const {
