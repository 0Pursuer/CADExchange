...
public:
    explicit SketchSegmentAccessor(std::shared_ptr<const CSketchSeg> seg = nullptr)
        : m_seg(std::move(seg)) {}

    bool IsValid() const {
        return m_seg != nullptr;
    }
    ...
    // --- 底层访问 ---
    std::shared_ptr<const CSketchSeg> GetRaw() const {
        return m_seg;
    }
};
...
public:
    ...
    explicit SketchAccessor(std::shared_ptr<FeatureAccessorBase> feat)
        ...
        if (feat) m_sketch = std::dynamic_pointer_cast<const CSketch>(feat->GetRaw());
    }

    bool IsValid() const {
        return m_sketch != nullptr;
    }

    // --- 参考平面 ---
    ReferenceAccessor GetReferencePlane() const {
        if (IsValid()) {
            return ReferenceAccessor(m_sketch->referencePlane);
        }
        return ReferenceAccessor(nullptr);
    }

    bool HasReferencePlane() const {
        return IsValid() && m_sketch->referencePlane != nullptr;
    }
    ...
    SketchSegmentAccessor GetSegmentByLocalID(const std::string& localID) const {
        if (!IsValid()) return SketchSegmentAccessor(nullptr);
